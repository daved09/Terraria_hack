...
namespace SLS
{
    class ProjectileAI
    {
        public static void ProjectTile_AI001Post(Projectile projectile)
        {
            if (projectile.type == ProjectileID.LunarFlare) return;
            if (projectile.type == ProjectileID.NebulaBlaze1) return;
            if (projectile.type == ProjectileID.NebulaBlaze2) return;
            if (projectile.type == ProjectileID.ChlorophyteBullet) return; // don't want to do tracking x2
            if (projectile.type == ProjectileID.VortexBeaterRocket) return; // don't want to do tracking x2
            if (projectile.type == ProjectileID.PygmySpear) return;
            if (projectile.type == ProjectileID.MiniRetinaLaser) return;
            if (projectile.type == ProjectileID.ElectrosphereMissile) return;
            if (projectile.type == ProjectileID.Meteor1) return;
            if (projectile.type == ProjectileID.Meteor2) return;
            if (projectile.type == ProjectileID.Meteor3) return;
            if (projectile.type == ProjectileID.MoonlordArrow) return;
            if (projectile.type == ProjectileID.MoonlordArrowTrail) return;
            if (projectile.type == ProjectileID.MiniSharkron) return;
